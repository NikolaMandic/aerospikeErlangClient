...
%% a (configurable) number operations on the Aerospike Citrusleaf Erlang
%% Client.  It's purpose is to demonstrate both the AS CL Erlang Client API
...
%% Include the Citrusleaf structures
...
%% Import the Citrusleaf API module and Define those functions from it that 
...
%% ---------------------------------------------
%% Process the results of a Citrusleaf Function
...
%% {  citrusleaf_error , { Number, Msg } }
%% Not sure if we'll ever see an error without a message.
processResults( { citrusleaf_error, _ErrorList},  read, _StatsRecord ) ->
...
processResults( citrusleaf_error, _Op, _StatsRecord ) ->
% io:format("[~s]:General ERROR(~p) Op(~p) ~n ",
%   ["ProcessResults", citrusleaf_error, _Op ]),
...
%         io:format("Do Op: ProcSeq(~p) GET(~s) ~n", [_ProcSeq,Key]),
          ReadResult = citrusleaf:getAll(_C, _Ns, _Set, Key, 0),
...
%           [_ProcSeq,Key,Record]),
        WriteResult =
          citrusleaf:put(_C, _Ns, _Set, Key, Record, 0),
